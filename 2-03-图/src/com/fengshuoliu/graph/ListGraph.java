package com.fengshuoliu.graph;

import com.fengshuoliu.MinHeap;
import com.fengshuoliu.UnionFind;
...
public class ListGraph<V, E> extends Graph<V, E> {
    ...
    private Map<V, Vertex<V, E>> vertices = new HashMap<>(); // 用来存放所有的顶点
    private Set<Edge<V, E>> edges = new HashSet<>();         // 用来存放所用的边
    ...
    @Override
    public int edgesSize() { // 边数
        return edges.size();
    }

    @Override
    public int verticesSize() { // 顶点数
    ...
    @Override
    public void addEdge(V from, V to) { // 添加边
        addEdge(from, to, null);
    }

    @Override
    public void addEdge(V from, V to, E weight) { // 添加有权值的边
        // 判断 from 、to顶点是否存在
    ...
    @Override
    public void removeEdge(V from, V to) { // 删除边
        Vertex<V, E> fromVertex = vertices.get(from);
    ...
    @Override
    public void bfs(V begin, VertexVisitor<V> visitor) { // 广度优先搜索(Breadth First Search)
        if (visitor == null) return;
        Vertex<V, E> beginVertex = vertices.get(begin);
        if (beginVertex == null) return;

        Set<Vertex<V, E>> visitedVertices = new HashSet<>(); // 存放已经访问的节点
    ...
    @Override
    public void dfs(V begin, VertexVisitor<V> visitor) { // 深度优先搜索(Depth First Search) - 非递归
        if (visitor == null) return;
        Vertex<V, E> beginVertex = vertices.get(begin);
        if (beginVertex == null) return;

        Set<Vertex<V, E>> visitedVertices = new HashSet<>(); // 存放已经访问过的顶点
    ...
    public List<V> topologicalSort() {
        List<V> list = new ArrayList<>(); // 存放返回结果
        Queue<Vertex<V, E>> queue = new LinkedList<>();
        Map<Vertex<V, E>, Integer> ins = new HashMap<>(); // 记录入度不为 0 的
        // 初始化（将入度为0的节点都放入队列）
        vertices.forEach((V v, Vertex<V, E> vertex) -> {
            int in = vertex.inEdges.size(); // 入度值
            if (in == 0) {
                queue.offer(vertex);
            } else {
                ins.put(vertex, in);
        ...
        while (!queue.isEmpty()) {
            ...
            for (Edge<V, E> edge : vertex.outEdges) {
                int toIn = ins.get(edge.to) - 1;  // 入度减 1
                if (toIn == 0) {
                    queue.offer(edge.to);
                } else {
    ...
    private Set<EdgeInfo<V, E>> prim() {
        Iterator<Vertex<V, E>> it = vertices.values().iterator(); // 获取顶点信息
        if (!it.hasNext()) return null;
        Vertex<V, E> vertex = it.next(); // 顶点
        Set<EdgeInfo<V, E>> edgeInfos = new HashSet<>();   // 存边信息
        Set<Vertex<V, E>> addedVertices = new HashSet<>(); // 已经添加的顶点信息
        addedVertices.add(vertex);
        MinHeap<Edge<V, E>> heap = new MinHeap<>(vertex.outEdges, edgeComparator); // 使用堆来获取最值
        int verticesSize = vertices.size(); // 顶点数量
    ...
    private Set<EdgeInfo<V, E>> kruskal() {
        ...
        while (!heap.isEmpty() && edgeInfos.size() < edgeSize) {
            Edge<V, E> edge = heap.remove();
            if (uf.isSame(edge.from, edge.to)) continue; // 如果构成环(使用并查集)
            edgeInfos.add(edge.info());
            uf.union(edge.from, edge.to); // 将两个点加入最小生成树
        }
        return edgeInfos;
    }
